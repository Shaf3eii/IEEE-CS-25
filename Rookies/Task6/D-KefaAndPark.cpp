...
int edges, noMoreThanConsecutiveCats;
...
int dfs(int node, int parent, int catsSoFar) {
    ...
    if (catsSoFar > noMoreThanConsecutiveCats) {return 0;}
    if (sz(adj[node]) == 1 and node != 1) {return 1;} // leaf node (restaurant)


    int restaurants = 0;
    for (auto neighbour : adj[node]) {
        if (neighbour == parent) {continue;}
        restaurants += dfs(neighbour, node, catsSoFar);
    }
    return restaurants;
}

void Erz3(int testcase) {
    cin >> edges >> noMoreThanConsecutiveCats;
    for (int i = 1; i <= edges; i++) {
        cin >> isCat[i];
    }
    for (int i = 1, u, v; i < edges; ++i) {
        cin >> u >> v;
        adj[u].push_back(v);
        adj[v].push_back(u);
    }
