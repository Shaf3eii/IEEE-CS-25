...
int numberOfFriends, maxCostPerTravels;
vector<pair<int, int>> adjList[1005];
bool visited[1005];

void dfs(const int node, int costSoFar) {
    if (visited[node]) return;
    visited[node] = true;
    maxCostPerTravels = max(maxCostPerTravels, costSoFar);
    for (auto neighbour : adjList[node]) {
        auto [curCost, next] = neighbour;
        dfs(next, costSoFar + curCost);
    }
    // visited[node] = false;
}
...
void Erz3(int testcase) {
    cin >> numberOfFriends;
    for (int i = 0, from, to, cost; i < numberOfFriends - 1; i++) {
        cin >> from >> to >> cost;
        adjList[from].emplace_back(cost, to);
        adjList[to].emplace_back(cost, from);
    }
    dfs(0, 0);
    cout << maxCostPerTravels << "\n";
}
